...
public class RidesCommandHandlerTests
{
    ...
    public void Reset_with_unknown_sequence_prints_error()
    {
        ...
        handler.Execute(["reset", "--sequence", "ceres"]);

        Assert.That(output.Lines, Has.Some.Contains("unknown identity profile 'ceres'"));
        Assert.That(output.Lines, Has.None.EqualTo("Success."));
    }

    [TestCase("--profile")]
    [TestCase("--sequence")]
    public void Reset_neptune_writes_only_canonical_identity_and_zero_ride_blocks(string option)
    {
        var output = new StringBuilderRidesOutput();
        var pm3 = FakeRidesPm3Api.WithSequenceRides(EncodingSequences.Mercury, 73);
        var handler = new RidesCommandHandler(pm3, output, new RidesConfig(), new ScriptedRidesInput("y"));

        handler.Execute(["reset", option, "neptune"]);
        ...
        Assert.That(pm3.GetBlockHex(1), Is.EqualTo("8BFE002A"));
        Assert.That(pm3.GetBlockHex(2), Is.EqualTo("F100C6A2"));
        Assert.That(pm3.GetBlockHex(3), Is.EqualTo("95D15917"));
        Assert.That(pm3.GetBlockHex(4), Is.EqualTo("95D15917"));
        Assert.That(pm3.GetBlockHex(5), Is.EqualTo("8F1249B0"));
    ...
    [Test]
    public void Reset_uranus_profile_writes_canonical_identity_blocks_and_zero_ride_encoding()
    {
        var output = new StringBuilderRidesOutput();
        var pm3 = FakeRidesPm3Api.WithSequenceRides(EncodingSequences.Mercury, 73);
        var handler = new RidesCommandHandler(pm3, output, new RidesConfig(), new ScriptedRidesInput("y"));

        handler.Execute(["reset", "--profile", "uranus"]);

        Assert.That(output.Lines, Has.Some.EqualTo("Success."));
        Assert.That(pm3.WrittenBlocks, Is.EqualTo(new uint[] { 1, 2, 3, 4, 5, 6 }));
        Assert.That(pm3.WrittenBlocks, Has.None.EqualTo(0));
        Assert.That(pm3.WrittenBlocks, Has.None.EqualTo(7));
        Assert.That(pm3.GetBlockHex(1), Is.EqualTo("FBFE002A"));
        Assert.That(pm3.GetBlockHex(2), Is.EqualTo("F1003C92"));
        Assert.That(pm3.GetBlockHex(3), Is.EqualTo("F5D1D766"));
        Assert.That(pm3.GetBlockHex(4), Is.EqualTo("F5D1D766"));
        Assert.That(pm3.GetBlockHex(5), Is.EqualTo("891249D0"));
        Assert.That(pm3.GetBlockHex(6), Is.EqualTo("891249D0"));
    }

    [Test]
    public void Reset_uranus_sequence_alias_uses_the_resettable_profile()
    {
        var output = new StringBuilderRidesOutput();
        var pm3 = FakeRidesPm3Api.WithSequenceRides(EncodingSequences.Mercury, 73);
        var handler = new RidesCommandHandler(pm3, output, new RidesConfig(), new ScriptedRidesInput("y"));

        handler.Execute(["reset", "--sequence", "uranus"]);
    ...
    {
        var output = new StringBuilderRidesOutput();
        var pm3 = FakeRidesPm3Api.WithSequenceRides(EncodingSequences.Venus, 73);
        var handler = new RidesCommandHandler(pm3, output, new RidesConfig());
        handler.Execute(["read"]);

        handler.Execute(["set", "100"]);

        var expected = EncodingSequences.Venus.Encode(100);
    ...
    {
        var output = new StringBuilderRidesOutput();
        var pm3 = FakeRidesPm3Api.WithSequenceRides(EncodingSequences.Venus, 120);
        var handler = new RidesCommandHandler(pm3, output, new RidesConfig());
        handler.Execute(["read"]);

        handler.Execute(["add", "10"]); // 120 + 10 = 130

        var expected = EncodingSequences.Venus.Encode(130);
        Assert.That(pm3.GetBlockHex(5), Is.EqualTo(expected.ToHex()));
        Assert.That(pm3.GetBlockHex(6), Is.EqualTo(expected.ToHex()));
    ...
    {
        var output = new StringBuilderRidesOutput();
        var pm3 = FakeRidesPm3Api.WithSequenceRides(EncodingSequences.Venus, 255);
        var handler = new RidesCommandHandler(pm3, output, new RidesConfig());
        handler.Execute(["read"]);
    ...
    {
        var output = new StringBuilderRidesOutput();
        var pm3 = FakeRidesPm3Api.WithSequenceRides(EncodingSequences.Venus, 499);
        var handler = new RidesCommandHandler(pm3, output, new RidesConfig());
        handler.Execute(["read"]);
    ...
    {
        var output = new StringBuilderRidesOutput();
        var pm3 = FakeRidesPm3Api.WithSequenceRides(EncodingSequences.Venus, 250);
        var handler = new RidesCommandHandler(pm3, output, new RidesConfig());
        handler.Execute(["read"]);
    ...
    {
        var output = new StringBuilderRidesOutput();
        var pm3 = FakeRidesPm3Api.WithSequenceRides(EncodingSequences.Venus, 180);
        var handler = new RidesCommandHandler(pm3, output, new RidesConfig(), new ScriptedRidesInput("y"));
    ...
    [Test]
    public void Jupiter_blocks_are_recognized_without_the_canonical_identity_and_set_preserves_jupiter()
    {
        var output = new StringBuilderRidesOutput();
        var pm3 = FakeRidesPm3Api.WithBlocks5And6(EncodingSequences.Jupiter.Encode(57), EncodingSequences.Jupiter.Encode(57));
        var handler = new RidesCommandHandler(pm3, output, new RidesConfig { PricePer100 = 4m });

        handler.Execute(["read"]);
        Assert.That(output.Lines, Has.Some.EqualTo("rides remaining: 57"));
        Assert.That(output.Lines, Has.Some.EqualTo("sequence: jupiter"));

        handler.Execute(["price", "set", "128"]);
        Assert.That(output.Lines, Has.Some.EqualTo("will cost: 2.84 EUR"));
        handler.Execute(["set", "128"]);
        Assert.That(pm3.GetBlockHex(5), Is.EqualTo("8C12C900"));
        Assert.That(pm3.GetBlockHex(6), Is.EqualTo("8C12C900"));
    }

    [Test]
    public void Jupiter_add_crosses_all_observed_counter_boundaries()
    {
        foreach (var (start, added, expected) in new[]
                 {
                     (7u, 1, 8u), (127u, 1, 128u), (255u, 1, 256u), (383u, 1, 384u), (499u, 1, 500u),
                 })
        {
            var output = new StringBuilderRidesOutput();
            var pm3 = FakeRidesPm3Api.WithBlocks5And6(EncodingSequences.Jupiter.Encode(start), EncodingSequences.Jupiter.Encode(start));
            var handler = new RidesCommandHandler(pm3, output, new RidesConfig());
            handler.Execute(["read"]);
    ...
    [Test]
    public void Uranus_blocks_are_recognized_without_the_canonical_identity_and_read_reports_uranus_sequence()
    {
        var output = new StringBuilderRidesOutput();
        var pm3 = FakeRidesPm3Api.WithBlocks5And6(EncodingSequences.Uranus.Encode(107), EncodingSequences.Uranus.Encode(107));
        var handler = new RidesCommandHandler(pm3, output, new RidesConfig());

        handler.Execute(["read"]);
        Assert.That(output.Lines, Has.Some.EqualTo("rides remaining: 107"));
        Assert.That(output.Lines, Has.Some.EqualTo("sequence: uranus"));
    }

    [Test]
    public void Uranus_set_preserves_uranus_encoding_after_writes()
    {
        var output = new StringBuilderRidesOutput();
        var pm3 = FakeRidesPm3Api.WithBlocks5And6(EncodingSequences.Uranus.Encode(127), EncodingSequences.Uranus.Encode(127));
        var handler = new RidesCommandHandler(pm3, output, new RidesConfig());
        handler.Execute(["read"]);

        handler.Execute(["set", "128"]);
        Assert.That(pm3.GetBlockHex(5), Is.EqualTo("8912C950"));
        Assert.That(pm3.GetBlockHex(6), Is.EqualTo("8912C950"));
        Assert.That(pm3.GetBlockHex(5), Is.Not.EqualTo(EncodingSequences.Saturn.Encode(128).ToHex()));
    }

    [Test]
    public void Uranus_add_crosses_observed_counter_boundaries()
    {
        foreach (var (start, added, expected) in new[]
                 {
                     (127u, 1, 128u), (255u, 1, 256u), (383u, 1, 384u),
        ...
        {
            var output = new StringBuilderRidesOutput();
            var pm3 = FakeRidesPm3Api.WithBlocks5And6(EncodingSequences.Uranus.Encode(start), EncodingSequences.Uranus.Encode(start));
            var handler = new RidesCommandHandler(pm3, output, new RidesConfig());
            handler.Execute(["read"]);
            handler.Execute(["add", added.ToString()]);
            Assert.That(pm3.GetBlockHex(5), Is.EqualTo(EncodingSequences.Uranus.Encode(expected).ToHex()), $"{start} + {added}");
            Assert.That(pm3.GetBlockHex(6), Is.EqualTo(EncodingSequences.Uranus.Encode(expected).ToHex()), $"{start} + {added}");
        }
    }

    [Test]
    public void Neptune_blocks_on_noncanonical_identity_are_recognized_and_set_preserves_neptune()
    {
        var output = new StringBuilderRidesOutput();
        var pm3 = FakeRidesPm3Api.WithBlocks5And6(EncodingSequences.Neptune.Encode(127), EncodingSequences.Neptune.Encode(127));
        var handler = new RidesCommandHandler(pm3, output, new RidesConfig());

        handler.Execute(["read"]);
        Assert.That(output.Lines, Has.Some.EqualTo("rides remaining: 127"));
        Assert.That(output.Lines, Has.Some.EqualTo("sequence: neptune"));

        handler.Execute(["set", "128"]);
        Assert.That(pm3.GetBlockHex(5), Is.EqualTo("8F12C930"));
        Assert.That(pm3.GetBlockHex(6), Is.EqualTo("8F12C930"));
    }

    [Test]
    public void Neptune_add_crosses_observed_counter_boundaries()
    {
        ...
        {
            var pm3 = FakeRidesPm3Api.WithBlocks5And6(EncodingSequences.Neptune.Encode(start), EncodingSequences.Neptune.Encode(start));
