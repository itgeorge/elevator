...
- Truthful connection state (2026-09-17): automatic launch discovery now publishes `Searching for saved bridge…`, not `Connected`, until token-bound proof plus authenticated status succeeds. If the verified service advertisement disappears, the credential remains paired but Connection returns to searching without sending a request; a later re-advertisement is treated as a new lifecycle event and must pass proof/status again before restoring Connected. Manual browsing does not downgrade an independently authenticated connection, while stop/denial/failure return an unverified saved pairing to `Saved pairing restored`. Focused Bonjour tests passed 28/28, the full simulator suite passed 151/151, Release simulator build and `git diff --check` passed.
