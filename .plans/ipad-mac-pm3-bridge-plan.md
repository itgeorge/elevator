...
- Notes (2026-09-18, plan-end real Proxmark3 hardware validation — user away, card left on antenna):
  ...
  - **Skipped (user away):** network-loss mid-write, fresh pairing/QR, identity-mismatch reset seed on real hardware.
  ...
  - **PM3-unavailable validation (2026-09-18, Proxmark3 USB disconnected — bridge reachable):**
    - Preconditions: no `/dev/cu.usbmodem*` nodes; port `:5080` free before launch; **no writes**.
    - Serial absent: `ls /dev/cu.usbmodem*` → no such file.
    - Bridge startup: `dotnet run --project RidesBridge/RidesBridge.csproj -- --everyday` **succeeds** (device `StartAsync` does not open USB at startup; PM3 connect is lazy on first hardware request). Bound `http://0.0.0.0:5080`; reachable `http://192.168.0.163:5080/` (also `http://10.2.0.2:5080/`, `http://169.254.197.217:5080/`).
    - Authenticated localhost scan (`POST /api/v1/pair` one-time PIN → bearer): `GET /api/v1/hardware/page0/scan` → HTTP **503** `{"code":"pm3_unavailable","message":"Proxmark3 is unavailable."}` — **not** HTTP 409 `no_chip`. **No** `/api/v1/hardware/page0/missing` or `/api/v1/hardware/page0/mutations` in normal detect flow.
    - Unauthenticated scan: HTTP **401**.
    - Concept A on iPad (same Wi‑Fi, bridge up): detect via `NetworkRideTokenDevice` maps 503 `pm3_unavailable` to `.failed("Scan failed: Bridge error pm3_unavailable: Proxmark3 is unavailable.")` — distinct from `.noChip` / `RidesViewModel.noChipMessage` and from `BridgeClientError.unreachable` when the bridge HTTP server is reachable.
    - **Fake PM3 pm3-unavailable profile:** `RIDES_FAKE_PM3_PROFILE=pm3-unavailable dotnet run --project RidesBridge/RidesBridge.csproj -- --fake-pm3` (aliases: `pm3_unavailable`, `unavailable`) reproduces the physical disconnected-USB scan contract without exotic hardware (`FakePm3Device.CreateUnavailable()` → HTTP 503 `pm3_unavailable`; host coverage in `Page0ScanBridgeTests.FakePm3UnavailableScanReturns503Pm3Unavailable` and `FakePm3LaunchTests.ProfileResolverAcceptsDocumentedAliases`).
  ...
  - **Unknown-dump validation (2026-09-18, unrecognized physical token on `/dev/cu.usbmodem1301`):**
    - Preconditions: serial free before launch; token on antenna expected to be unknown/unregistered family; **read-only** (scan + missing only; **no** `/mutations`; blocks 0/7 never written).
    - Bridge: `dotnet run --project RidesBridge/RidesBridge.csproj -- --everyday` on `http://192.168.0.163:5080/` (API `v1` / bridge `1.0.0`).
    - **Observed raw values (antenna coupling unstable — multiple snapshots; best full read below):**
      - Successful localhost `scan` (HTTP 200): block4 `5BA494A3`, block5 `1D12265E`, block6 `1D12265E`, signal `62854` mV.
      - Matching localhost `missing` (HTTP 200, once): block0 `00148040`, block1 `43FE0062`, block2 `5BA494A3`, block3 `2CF11806`, block7 `96788C03`.
      - iPad-successful assembled dump (from persisted filename): `5BA494A3` · `43FE0062` · `5BA494A3` · `5BA494A3` · `2CF11806` · `1D12265E` · `1D12265E` · `96788C03`.
      - Intermittent failures also observed: HTTP **409** `no_chip` on `scan` or `missing` when coupling dropped; one early run HTTP **504** `pm3_timeout` on `lf tune` (maps to bad-read class, distinct from undecodable mirrors).
      - Mirrors `1D12265E`/`1D12265E` do **not** match fake `unknown` profile (`DEADBEEF`/`FACECAFE`, block4 `00000004`).
    - **Fake vs physical:** fake `RIDES_FAKE_PM3_PROFILE=unknown` covers the **undecodable-mirrors** Concept A path deterministically; this physical token exercises the same HTTP/workflow shape with real mirror bytes. Intermittent `no_chip` / `pm3_timeout` on physical hardware are distinct bad-read classes (already covered by fake `no-chip`, `read-failed`, `tune-failed` profiles).
    - **iPad Concept A (`RIDES_UNKNOWN_DETECT=1`, `RIDES_BRIDGE_ADDRESS_OVERRIDE=192.168.0.163:5080`, existing Keychain credential):** Debug build `/tmp/RidesTabletUnknownDerived`, team `TJY5296P7S`, bundle `com.itgeorge.RidesTablet`. Successful run HTTP: `GET /api/v1/pair/status` (200) → `GET /api/v1/hardware/page0/scan` (200) → `GET /api/v1/hardware/page0/missing` (200) — exactly one `missing`; **no** `/mutations`. DEBUG console: `RIDES_UNKNOWN_DETECT_SUCCESS state=unknown message=Unknown, logged dump=elevator-t55xx-5BA494A3-43FE0062-5BA494A3-5BA494A3-2CF11806-1D12265E-1D12265E-96788C03--rides-UNKNOWN.bin`.
    - **New code (DEBUG only, left uncommitted):** `RIDES_UNKNOWN_DETECT=1` one-shot harness in `BridgeConnectionLaunchConfiguration`, `RidesRootView`, `BridgeConnectionModel.runLaunchUnknownDetectIfRequested()` (mirrors `RIDES_NOCHIP_DETECT`).
