...
- writes a page-0 `.bin` into `ride-capture-data/dumps/yyyy-MM-dd/` from the parsed dump output; legacy externally saved PM3 dumps found in the configured search directory are copied instead
...
Known token identities include canonical reset profiles such as `mercury`, `venus`, `earth`, `pluto`, and `mars`, plus validated identity variants registered in `TokenIdentityProfiles` (for example `venus21ff` and `earth-a457`). These recognized identities do not produce `UNKNOWN_TOKEN`, even though variant profiles do not yet have production reset images.
