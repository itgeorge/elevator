...
1. Optionally open the top-right slider menu to choose a fake reader simulation scenario.
...
5. Tap **Charge token** only after a known token has changed. A successful fake write makes pending the new current count.
...
For a local UI review, launch the app on the same iPad Air (4th generation) simulator and exercise the simulation scenarios from the top-right menu.
